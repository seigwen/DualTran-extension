...
Promise.all([twpConfig.onReady(), getTabHostName()]).then(function (_) {
  ...
  /**
   * Check if a node was generated by DualTran itself (not page content).
   * Used by the MutationObserver to filter out DualTran-generated DOM changes
   * so they are not fed back into the translation engine as "new content".
   *
   * Currently detects:
   * - .dualtran-aitranslatedtext-replacemode: AI translation result span (replaceOriginal mode)
    ...
    mutations.forEach((mutation) => {
      ...
      mutation.addedNodes.forEach((addedNode) => {
        ...
        if (nodeName.toLowerCase() !== "translated" && !isDescendantOfTranslated(addedNode)) {
          ...
          if (isDualTranGeneratedNode(addedNode)) {
            return;
          }
  ...
  pageTranslator._setForceAiTranslation = (v) => { shouldForceAiAfterPageTranslation = v; };
   ...
   /** @internal — for testing MutationObserver DualTran-generated node filtering */
   pageTranslator._isDualTranGeneratedNode = isDualTranGeneratedNode;
